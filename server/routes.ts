...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/admin/stats", authenticateToken, requireRole("admin"), async (req, res) => {
    try {
      ...
      // DEBUG: Write roles to file
      // DEBUG: Write roles to file
      const fs = await import('fs');
      fs.writeFileSync('roles_dump.txt', JSON.stringify(allUsers.map(u => ({ id: u.id, role: u.role, username: u.username })), null, 2));
  ...
  app.delete("/api/admin/classes/:id", authenticateToken, requireRole("admin"), async (req, res) => {
    try {
      ...
      const fs = require('fs');
      fs.appendFileSync('server/error.log', `${new Date().toISOString()} - Delete Class Error: ${error.message}\n${JSON.stringify(error)}\n`);
      res.status(500).json({ message: error.message || "Server error" });
    }
  });
  ...
  app.delete("/api/admin/subjects/:id", authenticateToken, requireRole("admin"), async (req, res) => {
    try {
      ...
      const fs = require('fs');
      fs.appendFileSync('server/error.log', `${new Date().toISOString()} - Delete Subject Error: ${error.message}\n${JSON.stringify(error)}\n`);
      res.status(500).json({ message: error.message || "Server error" });
    }
  });
